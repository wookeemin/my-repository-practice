...
```bash
   Route 53 Sec: In bound : "SSH 22, HTTP 80  > anywhere(0:/00000)" # ec2
```
...
yum update -y
yum install -y httpd # apache server install 
yum install -y wget
cd /var/www/html
...
```
#jump to the step 3. creating static server in S3 bucket.

3. Create EC2 in default VPC as named "Geo-Frankfurt"

 ```bash 
Region: "N.Virginia"
VPC: Default VPC #same for all
Subnet: PublicA # no preference
Sec Group: "Route 53 Sec"
...
### STEP 3: Create a Static WebSite Hosting :(or use existing one from former session)
 ...
 1. Create Static WebSite Hosting-1/ "www.[your sub-domain name].net"  # route53demo.wookeemin.com 
    ...
    "Statement": [
        {
            ...
            "Principal": "*",
            "Action": "s3:GetObject",
            "Resource": "arn:aws:s3:::route53demo.wookeemin.com/*" #change to your domain name
        }
    ]
}
