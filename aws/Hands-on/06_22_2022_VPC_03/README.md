...
Elastic IP allocation ID  : Allocate Elastic IP # why? should be Endpoint, thus required
```
...
```bash
ssh -A ec2-user@ec2-3-88-199-43.compute-1.amazonaws.com #ssh -A ec2-user@ec2-50-19-57-156.compute-1.amazonaws.com 
```
### B.Connect to the Private Instance

- once logged into the bastion host connect to the private instance in the private subnet:
```bash
ssh ec2-user@[Your private EC2 private IP] ##ssh ec2-user@10.7.2.214
```
...
```bash
aws s3 cp s3://clarusway-vpc-endpoint/Honda.png . # aws s3 cp s3://clarusway-vpc-endpoint-wookee . #aws s3 cp s3://clarusway-vpc-endpoint-wookee/Honda.png .
```
